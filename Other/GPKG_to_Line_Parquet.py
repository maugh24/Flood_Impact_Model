...
def convert_geopackage_to_parquet(gpkg_file, output_parquet):
    ...
    Processes only LineString geometries, ignoring all other geometry types.
    ...
    print(f"\nExtracting LineString features from all layers into single Parquet file...")
    print("=" * 70)
    ...
    for layer_idx, layer_name in enumerate(available_layers, 1):
        try:
            ...
            linestring_mask = gdf.geometry.type == 'LineString'
            linestrings = gdf[linestring_mask].copy()

            if len(linestrings) == 0:
                continue

            # Only print for layers with LineString features
            print(f"\n[{layer_idx}/{len(available_layers)}] Processing layer: {layer_name}")
            print(f"  ✓ Loaded {len(gdf):,} features, filtered to {len(linestrings):,} LineString features")

            # Add layer name as a column
            linestrings['layer_name'] = layer_name

            # Store original geometry type
            linestrings['geometry_type'] = linestrings.geometry.type

            # Store CRS
            if linestrings.crs is not None:
                linestrings['crs'] = str(linestrings.crs)

            all_dataframes.append(linestrings)
            total_features += len(linestrings)
    ...
    if not all_dataframes:
        raise ValueError("No LineString features found in any layer!")
...
if __name__ == "__main__":
    ...
    output_parquet = r"C:\C_Drive_Brians_Stuff\Python_Projects\Files\OSM_Parquet\central-america-lines.parquet"
