...
def calculate_basin_buildings_from_parquet(basin_file, building_parquet, output_folder):
    ...
    if len(buildings_in_basins) == 0:
        ...
        # Print breakdown
        print("\n" + "=" * 60)
        print("Building type breakdown (top 20):")
        print("=" * 60)
        for _, row in type_counts.head(20).iterrows():
            print(f"  {row['building_type']}: {row['count']:,}")
...
if __name__ == "__main__":
    ...
    building_parquet = r"C:\C_Drive_Brians_Stuff\Python_Projects\Files\OSM_Parquet\central-america-polygons.parquet"
